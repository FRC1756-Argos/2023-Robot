...
AutonomousLeftSlamGrabBalance::AutonomousLeftSlamGrabBalance(SwerveDriveSubsystem& drive,
...
void AutonomousLeftSlamGrabBalance::Initialize() {
  bool blueAlliance = frc::DriverStation::GetAlliance() == frc::DriverStation::Alliance::kBlue;
  auto startingPosition =
      blueAlliance ? starting_positions::blue_alliance::leftSlamGrab : starting_positions::red_alliance::leftSlamGrab;
  // angle to drive to game piece
  units::degree_t angleToGamePiece = blueAlliance ? interim_waypoints::blue_alliance::slam_grab::left::angleToPickup :
                                                    interim_waypoints::red_alliance::slam_grab::left::angleToPickup;
  ...
  m_allCommands = InitializeOdometryCommand{&m_drive, {startingPosition}}.ToPtr().AndThen(
      DriveOverChargingStation(&m_drive, 0_deg, 0_deg).ToPtr());
...
void AutonomousLeftSlamGrabBalance::Execute() {
  m_allCommands.get()->Execute();
}

// Called once the command ends or is interrupted.
void AutonomousLeftSlamGrabBalance::End(bool interrupted) {
  if (interrupted) {
...
bool AutonomousLeftSlamGrabBalance::IsFinished() {
...
/**
   ...
   */
std::string AutonomousLeftSlamGrabBalance::GetName() const {
  return "Left slam grab balance";
}

/**
   * @copydoc AutonomousCommand::GetCommand()
   */
frc2::Command* AutonomousLeftSlamGrabBalance::GetCommand() {
