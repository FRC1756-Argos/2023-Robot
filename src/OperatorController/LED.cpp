...
LED::LED(uint8_t controlPin) : LED(controlPin, (bool)false) {}

LED::LED(uint8_t controlPin, bool pwmEnable) : LED(controlPin, NOT_A_PIN, pwmEnable) {}

LED::LED(uint8_t controlPin, uint8_t groundPin, bool pwmEnable)
...
void LED::SetPulse() {
  ...
  if (NOT_A_PIN != m_controlPin) {
    ...
    if (m_illuminate) {
      ...
      brightness = CalcPulseBrightness(PWM_MAX_BRIGHTNESS, period, true, PWM_MIN_BRIGHTNESS);
    }
...
uint8_t LED::CalcPulseBrightness(uint8_t brightness, uint16_t period, bool smooth, uint8_t offBrightness) {
  bool invert = false;
  period /= PERIODDIVISOR;
  uint32_t condensedNow = millis() / PERIODDIVISOR;
  if (smooth) {
    invert = (condensedNow % (2 * period)) != (condensedNow % period);
  }
